...
// face-api.js stub — the real package is only needed for the webcam
// face-tracking variant (enableWebcam=true). This component is mounted
// with enableWebcam={false} (the default) so none of these code paths
// ever execute. The stub satisfies the type-checker without dragging
// in the multi-MB face-api / TensorFlow.js bundle.
...
const faceapi = {
  nets: {
    tinyFaceDetector: {
      loadFromUri: (_: string): Promise<void> => Promise.resolve(),
    },
  ...
  TinyFaceDetectorOptions: class {
    constructor(_o: { inputSize: number; scoreThreshold: number }) {}
  },
  detectSingleFace: (_video: HTMLVideoElement, _opts: unknown) => ({
    withFaceLandmarks: (_: boolean): Promise<FaceApiResult | null> =>
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    // Cap dpr at 1.5 — this is the 3rd WebGL canvas on the page (Hero
    // robot + AboutFlythrough fragments are the others). Original cap
    // was 2; lower bound saves substantial GPU work on retina screens.
